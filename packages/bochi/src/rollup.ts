  ...
  watcher.on('event', (e) => {
    switch (e.code) {
      ...
      case 'BUNDLE_END':
        console.log(
          chalk.green(
            `created ${chalk.bold(e.output.map(relativeId).join(', '))} in ${chalk.bold(
              prettyMilliseconds(e.duration)
            )}`
          )
        )
        break
...
export async function build() {
  ...
  console.log(
    chalk.cyan(
      `bundles ${chalk.bold(relativeId(paths.input))} → ${chalk.bold(relativeId(paths.output))}...`
    )
  )
